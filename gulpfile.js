...
gulp.task("cf-purge", function (done) {
  return cp
    .spawn("cfcli", ["purge", "-d", "btek.cc"], { stdio: "inherit" })
    .on("close", done);
});

gulp.task("algolia-push", ["build-all"], function (done) {
  cp.spawn("bundle", ["exec", "jekyll", "algolia", "push"], {
    stdio: "inherit",
  }).on("close", done);
});
...
gulp.task("screenshot", function (done) {
  var child = cp.spawn("http-server", ["-p", "9000", "_site"], {
    stdio: "inherit",
  });
  ...
  return nightmare()
    ...
    .then(function (result) {
      return cp
        .spawn(
          "convert",
          ["-resize", "1200", "tmp/screenshot.png", "tmp/screenshot.jpg"],
          { stdio: "inherit" }
        )
        .on("close", function () {
          return cp
...
gulp.task("js-production", ["js-libs", "js-custom"], function (cb) {
  pump(
    [
      gulp.src(["js/libs.min.js", "js/custom.min.js"]),
...
gulp.task("deploy", ["algolia-push"], function (done) {
  cp.spawn("git", ["push"], { stdio: "inherit" }).on("close", done);
});
